...
1. 파트너스 링크 변환 + 썸네일 — **한 화면(파트너스 링크 탭)**: 링크 하나 입력 → [생성하기] → 변환(서버) + 썸네일(브라우저 확장) 자동 실행
2. API키 관리 — `?admin=<MANAGER_TOKEN>` 일 때만 보임, 팀원은 키 미노출 (웹앱/서버)

## 구조
- `app.py` Streamlit UI (`_is_manager()`로 URL 쿼리파라미터 판별·`convert_view`(링크변환+썸네일 위젯 통합)·`api_key_view`)
...
- `extension/` 썸네일 추출용 크롬 확장 (`background.js`가 웹앱의 `chrome.runtime.sendMessage` 요청 처리)
...
- 썸네일은 서버에서 쿠팡을 긁지 않는다(차단됨). 실제 추출은 `extension/background.js`가 브라우저의 백그라운드 탭에서 수행하고, 웹앱은 `chrome.runtime.sendMessage(EXTENSION_ID, ...)`로 결과만 받아 화면에 뿌린다 — 이미지가 서버를 거치지 않는다.
- 썸네일 위젯은 반드시 `st.components.v1.declare_component(path=...)`로 서빙할 것 — `components.v1.html()`(srcdoc iframe)은 프레임 URL이 항상 `about:srcdoc`이라 `externally_connectable` 매칭이 절대 안 되고 `chrome.runtime`이 주입되지 않는다(2026-07-02 근본원인, `DECISIONS.md` 참고).
- `extension/manifest.json`의 `key`(고정 확장 ID `algnnfjoiiepjinfalghfnmmpjedehdg`)와 `externally_connectable.matches`는 임의로 지우지 말 것 — 지우면 웹앱↔확장 통신이 끊긴다. 배포 도메인이 바뀌면 `matches`에 추가.
