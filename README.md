...
| 기능 | 어디서 | 이유 |
|---|---|---|
| **1. 파트너스 링크 변환** | 웹앱(서버) | HMAC 서명엔 Secret Key가 필요해 서버에만 보관 |
| **1-부속. 썸네일** | 크롬 확장(브라우저, 사람이 직접 클릭) | 쿠팡이 프로그램이 여는 탭만 차단하고 사람이 직접 여는 탭은 차단 안 함(실측 확인) — 웹앱은 "이 상품 페이지 열기" 버튼만 제공, 실제 추출·다운로드는 쿠팡 페이지에서 확장 아이콘을 직접 클릭 |
| **2. 슬롯·API키 관리 (매니저 전용)** | 웹앱(서버) | 여러 키를 슬롯으로 관리, 팀원에겐 이름만 노출 |

> **왜 이 구조인가:** 쿠팡은 서버에서의 페이지 접근을 강력히 차단(403)한다. 일반 요청·Jina·Apify 모두 실측에서 막혔다. 웹앱이 프로그램으로 몰래 여는 탭도 결국 같은 차단에 걸리는 것으로 확인됐다(간헐적 재현 후 근본원인 확정). 오직 **사람이 직접 열고 클릭하는 방식만** 안정적이라, 썸네일은 확장의 팝업(직접 클릭)으로 처리한다. 자세한 근거는 `DECISIONS.md` 참고.
...
그래서 웹앱은 자동 추출을 시도하지 않고, "이 쿠팡 상품 페이지 열기" 버튼만 제공한다.
사용자가 그 페이지에서 툴바의 확장 아이콘을 직접 클릭해 썸네일을 받는다.
...
| 파일 | 역할 |
|---|---|
| `app.py` | Streamlit UI (URL 쿼리파라미터로 매니저 판별·링크변환·API키관리·확장 설치 안내 모달) |
